...
describe("RegistrationForm", () => {
  it("affiche tous les champs du formulaire", () => {
    ...
    expect(screen.getByPlaceholderText(/date de naissance/i)).toBeInTheDocument();
  ...
  it("permet de remplir et soumettre le formulaire avec des données valides", () => {
    ...
    fireEvent.change(screen.getByPlaceholderText(/nom/i), { target: { value: "Jean Dupont" } });
    fireEvent.change(screen.getByPlaceholderText(/ville/i), { target: { value: "Paris" } });
    fireEvent.change(screen.getByPlaceholderText(/email/i), { target: { value: "jean@example.com" } });
    fireEvent.change(screen.getByPlaceholderText(/date de naissance/i), { target: { value: "1990-01-01" } });
